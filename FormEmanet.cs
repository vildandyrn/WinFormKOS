...
namespace WinFormKOS
{
    public partial class FormEmanet : Form
    {
        ...
        int getGecikmeBedeli()
        {
            int cezaTL = 1;
            int gunFark = 0;
            foreach (DataRow row in IDataBase.DataToDataTable("select * from emanetler where okuyucuId = @id and durum = 1 and aktif = 1", new SqlParameter("@id", SqlDbType.Int) { Value = okuyucuId }).Rows)
            {
